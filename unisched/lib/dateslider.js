...
DateSlider = Class.create({
    ...
    if(l_oDate.toString("yyyy-MM-ddTHH:mm:ss")>l_oDate2.toString("yyyy-MM-ddTHH:mm:ss")) {
      var temp = l_oDate;
      l_oDate = l_oDate2;
      l_oDate2 = temp;
    }
		...
		if(this.oStartField && this.oEndField) {
			this.oStartField.setValue(l_oDate.toString(Date.CultureInfo.formatPatterns.shortDate));		
			this.oEndField.setValue(l_oDate2.toString(Date.CultureInfo.formatPatterns.shortDate));		
		}
    ...
    if(l_panelLength<0) {
      l_panelLength = Math.abs(l_panelLength)-4;
      left: $('shiftpanel').setStyle({left : ($('righthandle').offsetLeft)+'px'});
    }
    ...
    if(l_panelLength<0) {
      l_panelLength = Math.abs(l_panelLength)-5;
      left: $('shiftpanel').setStyle({left : ($('righthandle').offsetLeft)+'px'});
    } else left: $('shiftpanel').setStyle({left: ($('lefthandle').offsetLeft+4)+'px'});
		$('shiftpanel').setStyle({width : l_panelLength+'px'});															
		sliderReference._setDates();		
	},
	...
	attachFields : function (p_oStartField, p_oEndField) {
		...
		p_oStartField.setValue(this.oStartDate.toString(Date.CultureInfo.formatPatterns.shortDate));
		p_oEndField.setValue(this.oEndDate.toString(Date.CultureInfo.formatPatterns.shortDate));
      ...
      if(l_oStartDate.toString("yyyy-MM-ddTHH:mm:ss")>l_oEndDate.toString("yyyy-MM-ddTHH:mm:ss")) {
        var temp = l_oStartDate;
        l_oStartDate = l_oEndDate;
        l_oEndDate = temp;
        p_oStartField.setValue(l_oStartDate.toString(Date.CultureInfo.formatPatterns.shortDate));
        p_oEndField.setValue(l_oEndDate.toString(Date.CultureInfo.formatPatterns.shortDate));
      }
